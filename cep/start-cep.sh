...
JAVA_OPTS="-Xms50M -Xmx128M -Djava.ext.dirs=lib"
MAIN_CLASS=com.zte.oespaas.cep.rules.api.CepRulesManagerApp
...
{
	cd $RUNHOME
	unzip -o -q $APP_PACKAGE
	CEP_PACKAGE=`basename $RUNHOME/*.jar`
}
...
start_app()
{
	...
	pid=$(nohup $JAVA $JAVA_OPTS -classpath $CEP_PACKAGE $MAIN_CLASS server ceprules.yml >> $LOGS_DIR/$LOG_FILE 2>&1 & echo $!)
